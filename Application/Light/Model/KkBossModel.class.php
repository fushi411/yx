...
class KkBossModel extends Model {
    ...
    public function login($id){
        /* 检测是否在当前应用注册 */
        $user = $this->field("id as kk_id,name,wxid,boss,rank as kk_rank,'kk' as system")->where(array('id'=>(int)$id))->find();
        $snglUser = M('yxhb_boss')->field("id as yxhb_id,rank as yxhb_rank,'yxhb' as system")->where(array('wxid'=>$user['wxid']))->find();
        $user['yxhb_id'] = $snglUser['yxhb_id'];
        $user['yxhb_rank'] = $snglUser['yxhb_rank'];
        if(!$user) {
