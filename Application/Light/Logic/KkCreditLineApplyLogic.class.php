...
class KkCreditLineApplyLogic extends Model {
    ...
    */
    ...
    public function submit(){
        ...
        $yeArr= $model->getkkline($user_id,$this->today);
        $oline = $yeArr;
        
        $aid = M('kk_creditlineconfig')->field('1')->group('clientid,dtime')->select();
        $aid = count($aid)+1;
        ...
        // 表单重复提交
        if(!M('kk_creditlineconfig')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('kk_creditlineconfig')->add($insertData);
        ...
        $wf = A('WorkFlow');
        $res = $wf->setWorkFlowSV('CreditLineApply', $aid, $salesid, 'kk');
