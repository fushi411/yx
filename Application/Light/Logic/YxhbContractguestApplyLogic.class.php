...
class YxhbContractguestApplyLogic extends Model {
    // 实际表名
    ...
    public function getCustomerList(){
        ...
        if (!empty($data)){
            $res = M('yxhb_newguest')->field('id,name as text')->where($map1)->where($where)->select();
        }else{
            $res = M('yxhb_newguest')->field('id,name as text')->where($map1)->select();
        }
        return $res;
    }
    ...
    public function getCustomerInfo(){
        ...
        $res = M('yxhb_newguest')->field('id,contacts,telephone,name,info')->where($map)->find();
    ...
    public function add(){
        ...
        $res = array(
            ...
            'g_stat3'=>2,               //新增为2；审批通过为1；删除为0；失效为3
            'g_date'=>$g_date,          //申请日期
            'g_stat4'=>0,
            'g_kpfs'=>$g_kpfs,          //开票方式
            'g_jsfs'=>$g_sljs,           //结算方式
        ...
        $result2 = M('yxhb_guest2')->add($res2);
        if(!$result2){
            $res = M('yxhb_guest2')->where(array('id'=>$result))->setField('g_stat3', 5);
        ...
        if (!empty($copyto_id)) {
            D('YxhbAppcopyto')->copyTo($copyto_id,'Contract_guest_Apply', $result);
        }
        $wf = A('WorkFlow');    //调用控制器
        $res = $wf->setWorkFlowSV('Contract_guest_Apply',$result, $salesid, 'yxhb');
