...
class KkContractApplyfmhLogic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_ht_fmh';
    ...
    public function recordContent($id)
    {
        $res = $this->record($id);
        $result = array();
        $clientname = M('kk_guest2_fmh')->field('g_name,reid')->where(array('id' => $res['ht_khmc']))->find();
        $topGys     = M('kk_guest2_fmh')->field('g_name')->where(array('id' => $clientname['reid']))->find();
    ...
    public function getYyHtml($data){
        ...
        $info = M('kk_ht_fmh')->field($field)->where($map)->order('ht_cate,ht_enday desc')->select();
    ...
    public function getDetailHtml($id){
        ...
        $info = M('kk_ht_fmh')->field($field)->where($map)->select();
        $info = $this->infoSort($info);
        if(empty($info)) return '无';
        $html = '';
        foreach($info as $k => $v){
    ...
    public function getCustomerList(){
        $keyWord = I('math');
        $model   = D('Guest');
        $data    = $model->getKkrecordUser_fmh($keyWord);
        return $data;
    }
    ...
    public function  getModel($id){
        ...
        $info = M('kk_guest2_fmh')->field($field)->where(array('id'=>$id))->find();
        return $info;
    }
...
//    }
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('ContractApply_fmh','');
        if(!$pro) return array('code' => 404,'msg' => '无审批流程,请联系管理员');
        if(!M('kk_ht_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $temp = array(
            'pid'         => $idArr[0],
            'ht_xyed'     => 0,
        ...
        $result = M('kk_ht_fmh')->addAll($addData);
        ...
        if(!empty($guest)){
            $map = array('id' => $id);
            M('kk_guest2_fmh')->where($map)->save($guest);
        }
        // 抄送
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('KkAppcopyto')->copyTo($copyto_id,'ContractApply_fmh', $idArr[0]);
        }
        $wf = A('WorkFlow');
        $salesid = session('kk_id');
        $res = $wf->setWorkFlowSV('ContractApply_fmh', $idArr[0], $salesid, 'kk');
    ...
    public function getPid(){
        $year  = date('Y',time());
        $count = M('kk_ht_fmh')->where('year(ht_stday)='.$year)->count();
        $id    = date('Ymd',time());
        $dh    = "XSHT".date("Y",time());
