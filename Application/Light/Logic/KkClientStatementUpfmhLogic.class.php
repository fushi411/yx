...
class KkClientStatementUpfmhLogic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_clientstatement_up_fmh';
    ...
    public function recordContent($id)
    {
        ...
        $data = M('kk_clientstatement_fmh')->where(array('id' => $res['aid']))->find();
        $result['content'][] = array('name'=>'客户名称：',
                                     'value'=>D('kk_guest2_fmh')->getName($data['client']),
                                     'type'=>'string',
                                     'color' => 'black'
                                    );
        ...
        $mydata = array(
            ...
            'stday' => $data['stday'],
            'enday' =>$data['enday'],
            'user_name' => D('kk_guest2_fmh')->getName($data['client']));
    ...
    public function sealNeedContent($id){
        ...
        $data = M('kk_clientstatement_fmh')->where(array('id' => $res['aid']))->find();
        $temp = array(
            array('title' => '客户名称' , 'content' => D('kk_guest2_fmh')->getName($data['client']) ),
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('ClientStatementUp_fmh','');
        if(!$pro) return array('code' => 404,'msg' => '无审批流程,请联系管理员');
        if(!M('kk_clientstatement_up_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $add = array(
            'aid' => $info['id'],
            'stat' => 2,
            'file' => $files,
        ...
        $result = M('kk_clientstatement_up_fmh')->add($add);
        if(!$result) return array('code' => 404,'msg' => '提交失败，请重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('KkAppcopyto')->copyTo($copyto_id,'ClientStatementUp_fmh', $result);
        }
        $wf = A('WorkFlow');
        $salesid = session('kk_id');
        $res = $wf->setWorkFlowSV('ClientStatementUp_fmh', $result, $salesid, 'kk');
    ...
    public function getList(){
        ...
        $data = M($system.'_clientstatement_fmh')->where($where)->order('dtime DESC')->limit((($page-1)*20).",20")->select();
    ...
    public function reInfo($data){
        ...
        foreach($data as $v){
            $temp = array();
            $temp['id'] = $v['id'];
            $temp['client_name'] = D('kk_guest2_fmh')->getName($v['client']);
    ...
    public function getInfo(){
        $id = I('post.id');
        $system    = I('post.system');
        $data = M($system.'_clientstatement_fmh')->where(array( 'id' => $id ))->select();
        $data = $this->reInfo($data);
        return $data[0];
    }
    /**
