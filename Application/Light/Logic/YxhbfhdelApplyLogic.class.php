...
class YxhbfhdelApplyLogic extends Model {

    protected $trueTableName = 'yxhb_fh';  // 实际表名
    /**
     * 记录内容
    ...
    public function recordContent($id)
    {
        ...
        $res3 = M("yxhb_fhdel")->order('del_date DESC')->where(array('fh_id'=>$res['id']))->find();    //发货删除记录表
        ...
        $result['imgsrc'] = '';
        $result['applyerID'] =  D('YxhbBoss')->getIDFromName($res3['del_person']);            //申请删除者的id
    ...
    public function transStat($id){
       $res = M('yxhb_fh')->where(array('id'=>$id))->find();
       if ($res['fh_stat'] == 1 && $res['fh_stat4'] == 1) return 2;                                       //审批中
       if ($res['fh_stat'] == 0) return 1;                                                                //审批通過
        if ($res['fh_stat'] == 1 && ($res['fh_stat4'] == 0 || $res['fh_stat4'] == 2)) return 0;          //撤销
    }
    ...
    public function fh_del(){
        ...
        $result = M('yxhb_fhdel')->add($addData);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请重新尝试！');
        $res = M('yxhb_fh')->where(array('id' =>$fh_id))->find();
        $data = array(
            0=>$res['fh_stat4'],
            1=>$res['fh_flag'],
        );
        ...
        M('yxhb_fh')->where(array('id' =>$fh_id))->save($save_fh);
        ...
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'fh_del_Apply', $fh_id,1);
        }
        $wf = A('WorkFlow');
        $salesid = session('yxhb_id');
        $res = $wf->setWorkFlowSV('fh_del_Apply', $fh_id, $salesid, 'yxhb');
