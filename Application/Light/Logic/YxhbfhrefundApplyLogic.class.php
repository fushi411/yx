...
class YxhbfhrefundApplyLogic extends Model {
    // 实际表名
    ...
    public function recordContent($id)
    {
        ...
        $result['applyerID'] =  D('YxhbBoss')->getIDFromName($res['fh_kpy']);
    ...
    public function delRecord($id)
    {
        ...
        M('yxhb_fh')->field(true)->where($map)->save($save_fh);
        $map = array('fh_num' => $res['fh_num']);
    ...
    public function getStat($id){
        ...
        $res = M('yxhb_appflowproc')->field('app_stat,app_name,app_stage')->where($where)->order('app_stat asc')->select();
        // 撤销
        $map = array('id' => $id);
   ...
   /**
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('YxhbAppflowtable')->havePro('fh_refund_Apply','');
        ...
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'fh_refund_Apply', $result,1);
        }
        $wf = A('WorkFlow');
