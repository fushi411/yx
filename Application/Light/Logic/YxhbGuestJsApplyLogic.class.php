...
class YxhbGuestJsApplyLogic extends Model {
    ...
    public function recordContent($id)
    {
        ...
        $data = M('yxhb_js')->field('sum(js_je) as js_je')->where(array('pid' => $id))->find();
        ...
        $result['mydata']['g_name']   = D('yxhb_guest2')->getParentName($res['client']);                      
        ...
        if($res['jslx'] == 5 ){
            ...
            $info = $this->getClientInfoDate($res['client'],'yxhb',$res['jl_date']);
    ...
    public function sealNeedContent($id){
        ...
        $temp = array(
            ...
            array('title' => '客户名称' , 'content' =>  D('yxhb_guest2')->getParentName($res['client'])),
    ...
    public function getCustomerList(){ 
        $data = I('math');
        $res = D('Guest')->getGuest('yxhb',$data,'GuestJsApply');
        return $res;
    }
    ...
    public  function getJcData($stday,$enday,$client){
        ...
        // 总客户获取
        $child = M('yxhb_guest2')->field('id,g_name')->where(array('_string' => "reid=$client or id=$client" , 'g_stat3' => 1))->select();
        $childData = array();
    ...
    public function submitDz(){
        ...
        $system    = I('post.system');
        // 流程检验
        // $pro = D('yxhbAppflowtable')->havePro('GuestJsApply','');
        ...
        if(!M('yxhb_js')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('yxhb_js')->add($insert);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请刷新页面重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('yxhbAppcopyto')->copyTo($copyto_id,'GuestJsApply', $pid);
        }
        // 签收通知
        $wf = A('WorkFlow');
        $salesid = session('yxhb_id');
        $res = $wf->setWorkFlowSV('GuestJsApply', $pid, $salesid, 'yxhb');
    ...
    public function submitJc(){
        ...
        if(!M('yxhb_js')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('yxhb_js')->addAll($insert);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请刷新页面重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
    ...
    public function getDh(){
        ...
        $data = M('yxhb_js')->where($map)->group('dh')->select();
