...
class YxhbAddMoneyQtLogic extends Model {
    // 实际表名
    protected $trueTableName = 'yxhb_feeqt';
    ...
    public function sealNeedContent($id){
        ...
        $result = array(
            'first_title'    => '收款方式',
            'first_content'  => $skfsArr[$res['nfkfs']],
            'second_title'   => '收款金额',
    ...
    public function submit(){
        ...
        if(!M('yxhb_feeqt')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('yxhb_feeqt')->add($feeData);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请刷新页面重新尝试！');
        // 抄送
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'AddMoneyQt', $result);
        }
        // 签收通知
        $all_arr = array();
    ...
    public function makeDh(){
        ...
        $res = M('yxhb_feeqt') ->where("date_format(jl_date, '%Y-%m-%d' )='$today' ")->count();
        $str = date('Ymd',time());
        $db  = "QT{$str}";
        $num = $res+1;
