...
class KkNewGuestApplyfmhLogic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_newguest_fmh';
    ...
    public function recordContent($id)
    {
        ...
        $result['imgsrc'] = '';
        $result['applyerID'] =  $res['salesid'];                                 //申请人的id
        $result['applyerName'] = $res['sales'];                                 //申请人的姓名
        $result['stat'] = $res['stat'];                                         //状态 0->撤销  1->审批通过  2->审批中
        return $result;
    }
    ...
    public function addNewGuest(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('NewGuestApply_fmh','');
        if(!$pro) return array('code' => 404,'msg' => '无审批流程,请联系管理员');
        //$_POST    提交数据的方式为post
        if(!M('kk_newguest_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');    //在Model.class.php中，自动表单令牌验证
        $result = M('kk_newguest_fmh')->add($res);

        if(!$result) return array('code' => 404,'msg' =>'提交失败，请重新尝试！');
        // 抄送
        $copyto_id = trim($copyto_id,',');      //移除字符串两侧的空白字符及','
        if (!empty($copyto_id)) {
            D('KkAppcopyto')->copyTo($copyto_id,'NewGuestApply_fmh', $result);
        }
        $wf = A('WorkFlow');    //调用控制器
        $res = $wf->setWorkFlowSV('NewGuestApply_fmh',$result, $salesid, 'kk');
    ...
    public function checkNameIsSet(){
        ...
        $data = M('kk_newguest_fmh')->where($map)->find();
