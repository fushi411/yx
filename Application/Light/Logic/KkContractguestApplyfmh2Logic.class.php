...
class KkContractguestApplyfmh2Logic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_guest2_fmh';
...
//        $html = $this->getEjHtml($res2);
    ...
    public function getCustomerList(){
        ...
        $sql1 = "select id,g_xmmc,g_name as text from kk_guest2_fmh where (reid = 0 and g_stat3 = 1) order by g_name ASC";
        $sql2 = "select id,g_xmmc,g_name as text from kk_guest2_fmh where (reid = 0 and g_stat3 = 1 and ( g_name like '%{$data}%' OR g_helpword like '%{$data}%') ) order by g_name ASC";
       if (!empty($data)){
           $res = M()->query($sql2);
       }else{
           $res = M()->query($sql1);
    ...
    public function getCustomerInfo(){
        ...
        $res = M('kk_guest2_fmh')->field('id,g_man,g_phone,g_khlx,g_kpfs,g_jsfs')->where($map)->find();
    ...
    function getzkh(){
        ...
        $res = M('kk_guest2_fmh')->field('id,g_name')->where($map)->select();
        if (empty($res)) return array('code' => 404,'msg' => '请重新刷新页面！');
        return array('code' => 200,'msg' => '提交成功' , 'data' =>$res);
    }
    ...
    function getzkhInfo(){
        ...
        $res = M('kk_guest2_fmh')->field('id,g_name,g_xmmc,g_man,g_phone,g_khlx,g_date,g_kpfs,g_jsfs')->where($map)->find();
        if (empty($res)) return array('code' => 404,'msg' => '请重新刷新页面！');
        return array('code' => 200,'msg' => '提交成功' , 'data' =>$res);
    }
    ...
    public function add(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('Contract_guest_Apply_fmh2','');
        if(!$pro) return array('code' => 404,'msg' => '无审批流程,请联系管理员');
        //$_POST    提交数据的方式为post
        if(!M('kk_guest2_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');    //在Model.class.php中，自动表单令牌验证
        $result = M('kk_guest2_fmh')->add($res);

        if(!$result) return array('code' => 404,'msg' =>'提交失败，请重新尝试！');
        // 抄送
        $copyto_id = trim($copyto_id,',');      //移除字符串两侧的空白字符及','
        if (!empty($copyto_id)) {
            D('KkAppcopyto')->copyTo($copyto_id,'Contract_guest_Apply_fmh2', $result);
        }
        $wf = A('WorkFlow');    //调用控制器
        $res = $wf->setWorkFlowSV('Contract_guest_Apply_fmh2',$result, $salesid, 'kk');
    ...
    public function add2($aid){
        ...
        $result = M('kk_guest2_fmh')->add($data);
        ...
        $data2 = M('kk_appflowproc')->where($map2)->find();
        unset($data2['id']);
        $data2['mod_name'] = 'Contract_guest_Apply2';
        $data2['aid'] = $result;
        $result2 = M('kk_appflowproc')->add($data2);
    ...
    public function getname(){
        ...
        $res = M('kk_guest2_fmh')->field('g_name')->where($map)->find();
    ...
    public function getsjkh($id){
        ...
        $res1 = M('kk_guest2_fmh')->field('reid')->where($map1)->find();
        $map2 = array(
            'id'=>$res1['reid'],
        );
        $res2 = M('kk_guest2_fmh')->field('g_name')->where($map2)->find();
        return $res2;
    }
    ...
    public function getzkhlist($id){
        ...
        $res2 = M('kk_guest2_fmh')->field('g_name')->where($map2)->select();
        ...
        if($num == 1 || empty($res2)){
            ...
            $data = M('kk_guest2_fmh')->field('g_name')->where($map2)->where(array('limit'=>$num-1))->select();
        }
        return $data;
    }
