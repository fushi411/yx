...
class YxhbClientStatementApplyLogic extends Model {
    ...
    public function recordContent($id)
    {
        ...
        $mydata = array(
            ...
            'stday' => $res['stday'],
            'enday' =>$res['enday'],
            'user_name' => D('yxhb_guest2')->getName($res['client']));
    ...
    public function sealNeedContent($id){
        ...
        $temp = array(
            array('title' => '客户名称' , 'content' => D('yxhb_guest2')->getName($res['client']) ),
    ...
    public function getCustomerList(){
        $data = I('math');
        $res = D('Guest')->getGuest('yxhb',$data,'ClientStatementApply');
        return $res;
    }
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('YxhbAppflowtable')->havePro('ClientStatementApply','');
        ...
        if(!M('yxhb_clientstatement')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('yxhb_clientstatement')->add($add);
        if(!$result) return array('code' => 404,'msg' => '提交失败，请重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'ClientStatementApply', $result);
        }
        $wf = A('WorkFlow');
        $salesid = session('yxhb_id');
        $res = $wf->setWorkFlowSV('ClientStatementApply', $result, $salesid, 'yxhb');
