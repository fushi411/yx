...
class YxhbContractApplyLogic extends Model {
    ...
    public function recordContent($id)
    {
        ...
        $result['imgsrc'] = '';
        $result['applyerID']   = D('YxhbBoss')->getIDFromName($res['ht_rdy']);
    ...
    public function getYyHtml($data){
        ...
        $info = M('yxhb_ht')->field($field)->where($map)->order('ht_cate,ht_enday desc')->select();
        $res   = array();
        $touch = array();   
        foreach( $info as $k=>$v){
            if(in_array($v['ht_cate'],$touch))continue;
            $touch[]      = $v['ht_cate'];
            $res[]        = $v;
        }
        $info = $res;
    ...
    public function getDetailHtml($id){
        ...
        $info = M('yxhb_ht')->field($field)->where($map)->select();
        if(empty($info)) return '无';
        $html = '';
        foreach($info as $k => $v){
            $html   .= "<input class='weui-input' type='text' style='color: black; font-weight: 700;border-bottom: 1px solid #e5e5e5; '  readonly value='{$v['ht_cate']}({$v['ht_wlfs']})'>";
    ...
    public function sealNeedContent($id){
        ...
        $result = array(
            ...
            'date'    => $res['ht_date'],
            'title'   => '上级客户',
            'name'    => $topGys['g_name']?$topGys['g_name']:'无', 
    ...
    public function  getModel($id){
        ...
        $info = M('yxhb_guest2')->field($field)->where(array('id' => $id))->find();
        
        return empty($info)?array('kf'=>'无','js'=> '无'):$info;
    }
    ...
    public function getStuff(){
        // select * from yxhb_config_cate_sn WHERE stat=2 ORDER BY id desc LIMIT 1
        $data   = M('yxhb_config_cate')->where('stat=2')->order('id desc')->find();
        $forArr = array('yhk','ehk','shk','sshk','whk','lhk','qhk','bhk','jhk');
        $res    = array();
        foreach($forArr as $v){
            if(in_array($data[$v],$res) || $data[$v] == '无' || !$data[$v]) continue;
            $res[] = $data[$v];
        }
        return $res;
    }
    ...
    public function submit(){
        ...
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'ContractApply', $idArr[0]);
        }
        $wf = A('WorkFlow');
        $salesid = session('yxhb_id');
        $res = $wf->setWorkFlowSV('ContractApply', $idArr[0], $salesid, 'yxhb');
    ...
    public function getPid(){
        $year  = date('Y',time());
        $count = M('yxhb_ht')->where('year(ht_stday)='.$year)->count();
        $id    = date('Ymd',time());
        $dh    = "XSHT".date("Y",time());
