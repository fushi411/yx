...
class KkGuestJsApplyfmhLogic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_js_fmh';
    ...
    public function recordContent($id)
    {
        ...
        $data = M('kk_js_fmh')->field('sum(js_je) as js_je')->where(array('pid' => $id))->find();
    ...
    public function getJcHtml($id){
        ...
        foreach($data as $vo){
            ...
            $enday  = $vo['js_enday'];
            $client = $vo['client'];
            $g_name = M('kk_guest2_fmh')->where(array('id' => $client))->find();
    ...
    public  function getJcData($stday,$enday,$client){
        ...
        // 总客户获取
        $child = M('kk_guest2_fmh')->field('id,g_name')->where(array('_string' => "reid=$client or id=$client" , 'g_stat3' => 1))->select();
        $childData = array();
    ...
    public function getJc($stday,$enday,$client){
        ...
        // 查询发货品种和包装
        $data = M('kk_fh_fmh')->field("fh_cate,if(fh_bzfs='散装','散装','袋装') as fh_bzfs,fh_bzfs as bzfs")->where($map)->group('fh_cate,fh_bzfs')->select();
        $temp = array();
        // 调价查询
    ...
    public function submitDz(){
        ...
        $system    = I('post.system');
        // 流程检验
        // $pro = D('kkAppflowtable')->havePro('GuestJsApply_fmh','');
        ...
        if(!M('kk_js_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('kk_js_fmh')->add($insert);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请刷新页面重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('kkAppcopyto')->copyTo($copyto_id,'GuestJsApply_fmh', $pid);
        }
        // 签收通知
        $wf = A('WorkFlow');
        $salesid = session('kk_id');
        $res = $wf->setWorkFlowSV('GuestJsApply_fmh', $pid, $salesid, 'kk');
    ...
    public function submitJc(){
        ...
        if(!M('kk_js_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('kk_js_fmh')->addAll($insert);
        if(!$result) return array('code' => 404,'msg' =>'提交失败，请刷新页面重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
    ...
    public function getDh(){
        ...
        $data = M('kk_js_fmh')->where($map)->group('dh')->select();
