...
class KkContractguestApplyfmhLogic extends Model {
    // 实际表名
    ...
    public function getCustomerList(){
       ...
       if (!empty($data)){
           $res = M('kk_newguest_fmh')->field('id,name as text')->where($map1)->where($where)->select();
       }else{
           $res = M('kk_newguest_fmh')->field('id,name as text')->where($map1)->select();
       }
        return $res;
    }
    ...
    public function getCustomerInfo(){
        ...
        $res = M('kk_newguest_fmh')->field('id,contacts,telephone,name,info')->where($map)->find();
    ...
    public function add(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('Contract_guest_Apply_fmh','');
        ...
        if (!empty($copyto_id)) {
            D('KkAppcopyto')->copyTo($copyto_id,'Contract_guest_Apply_fmh', $result);
        }
        $wf = A('WorkFlow');    //调用控制器
        $res = $wf->setWorkFlowSV('Contract_guest_Apply_fmh',$result, $salesid, 'kk');
        return array('code' => 200,'msg' => '提交成功' , 'aid' =>$result);
    }


    //粉煤灰合同客户的开票方式、结算方式转存到客户表中
    public function getdata(){
        $sql = "SELECT `id`,`ht_sljsfs`,`ht_kpfs`,`ht_khmc` FROM `kk_ht_fmh` WHERE `ht_stat` = 2";
        $res = M('kk_ht_fmh')->query($sql);;
        //修改子客户的数据
...
//                'reid'!=0,
            ...
            M('kk_guest2_fmh')->where($map)->data($data)->save();

            $map2 = array(
                'id'=>$val['ht_khmc'],
            );
            $reid = M('kk_guest2_fmh')->field('reid')->where($map2)->find();
            $map3 = array(
                'id'=>$reid['reid'],
                'g_stat3'=>1,
                'reid'=> 0,
            );
            M('kk_guest2_fmh')->where($map3)->data($data)->save();
        }
    }
