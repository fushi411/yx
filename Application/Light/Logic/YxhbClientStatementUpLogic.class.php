...
class YxhbClientStatementUpLogic extends Model {
    // 实际表名
    protected $trueTableName = 'yxhb_clientstatement_up';
    ...
    public function recordContent($id)
    {
        ...
        $data = M('yxhb_clientstatement')->where(array('id' => $res['aid']))->find();
        $result['content'][] = array('name'=>'客户名称：',
                                     'value'=>D('yxhb_guest2')->getName($data['client']),
                                     'type'=>'string',
                                     'color' => 'black'
                                    );
        ...
        $mydata = array(
            ...
            'stday' => $data['stday'],
            'enday' =>$data['enday'],
            'user_name' => D('yxhb_guest2')->getName($data['client']));
    ...
    public function sealNeedContent($id){
        ...
        $result = array(
            'first_title'    => '客户名称',
            'first_content'  => D('yxhb_guest2')->getName($data['client']),
            'second_title'   => '开始时间',
            'second_content' => $data['stday'],
            'third_title'    => '结束时间',
            'third_content'  => $data['enday'],
            'fourth_title'   => '相关说明',
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('YxhbAppflowtable')->havePro('ClientStatementUp','');
        if(!$pro) return array('code' => 404,'msg' => '无审批流程,请联系管理员');
        if(!M('yxhb_clientstatement_up')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $add = array(
            'aid' => $info['id'],
            'stat' => 2,
            'file' => $files,
        ...
        $result = M('yxhb_clientstatement_up')->add($add);
        if(!$result) return array('code' => 404,'msg' => '提交失败，请重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('YxhbAppcopyto')->copyTo($copyto_id,'ClientStatementUp', $result);
        }
        $wf = A('WorkFlow');
        $salesid = session('yxhb_id');
        $res = $wf->setWorkFlowSV('ClientStatementUp', $result, $salesid, 'yxhb');
    ...
    public function reInfo($data){
        ...
        foreach($data as $v){
            $temp = array();
            $temp['id'] = $v['id'];
            $temp['client_name'] = D('yxhb_guest2')->getName($v['client']);
