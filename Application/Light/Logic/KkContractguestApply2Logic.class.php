...
class KkContractguestApply2Logic extends Model {
    // 实际表名
...
//        );
    ...
    public function getCustomerList(){
        ...
        $sql1 = "select id,g_xmmc,g_name as text from kk_guest2 where (reid = 0 and g_stat3 = 1) order by g_name ASC";
        $sql2 = "select id,g_xmmc,g_name as text from kk_guest2 where (reid = 0 and g_stat3 = 1 and ( g_name like '%{$data}%' OR g_helpword like '%{$data}%') ) order by g_name ASC";
       if (!empty($data)){
           //$res = M('kk_guest2')->field('id,g_xmmc,g_name as text')->where($map1)->where($where)->where($where2)->select();
           $res = M()->query($sql2);
       }else{
           //$res = M('kk_guest2')->field('id,g_name as text')->where($map1)->where($where2)->select();
           $res = M()->query($sql1);
       }
        return $res;
    }
    ...
    public function getCustomerInfo(){
        ...
        $res = M('kk_guest2')->field('id,g_man,g_phone,g_khlx,g_kpfs,g_jsfs')->where($map)->find();
    ...
    function getzkhInfo(){
        ...
        $res = M('kk_guest2')->field('id,g_name,g_xmmc,g_man,g_phone,g_khlx,g_date,g_kpfs,g_jsfs')->where($map)->find();
        if (empty($res)) return array('code' => 404,'msg' => '请重新刷新页面！');
        return array('code' => 200,'msg' => '提交成功' , 'data' =>$res);
    }
    ...
    public function add(){
        ...
        if (!empty($copyto_id)) {
            D('KkAppcopyto')->copyTo($copyto_id,'Contract_guest_Apply2', $result);
        }
        $wf = A('WorkFlow');    //调用控制器
        $res = $wf->setWorkFlowSV('Contract_guest_Apply2',$result, $salesid, 'kk');
    ...
    public function getdata(){
        ...
        $sql = "SELECT `id`,`ht_sljsfs`,`ht_kpfs`,`ht_khmc` FROM `kk_ht` WHERE `ht_stat` = 2";
        $res = M('kk_ht')->query($sql);;
        //修改子客户的数据
...
//                'reid'!=0,
            ...
            $res = M('kk_guest2')->where($map)->data($data)->save();

            $map2 = array(
                'id'=>$val['ht_khmc'],
            );
            $reid = M('kk_guest2')->field('reid')->where($map2)->find();
            $map3 = array(
                'id'=>$reid['reid'],
                'g_stat3'=>1,
                'reid'=> 0,
            );
            $res2 = M('kk_guest2')->where($map3)->data($data)->save();
    ...
    public function getname(){
        ...
        $res = M('kk_guest2')->field('g_name')->where($map)->find();
    ...
    public function getsjkh($id){
        ...
        $res1 = M('kk_guest2')->field('reid')->where($map1)->find();
        $map2 = array(
            'id'=>$res1['reid'],
        );
        $res2 = M('kk_guest2')->field('g_name')->where($map2)->find();
        return $res2;
    }
