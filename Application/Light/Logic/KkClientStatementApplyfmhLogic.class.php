...
class KkClientStatementApplyfmhLogic extends Model {
    // 实际表名
    protected $trueTableName = 'kk_clientstatement_fmh';
    ...
    public function recordContent($id)
    {
        ...
        $result['content'][] = array('name'=>'客户名称：',
                                     'value'=>D('kk_guest2_fmh')->getName($res['client']),
                                     'type'=>'string',
                                     'color' => 'black'
                                    );
        ...
        $mydata = array(
            ...
            'stday' => $res['stday'],
            'enday' =>$res['enday'],
            'user_name' => D('kk_guest2_fmh')->getName($res['client']));
    ...
    public function sealNeedContent($id){
        ...
        $temp = array(
            array('title' => '客户名称','content' => D('kk_guest2_fmh')->getName($res['client']) ),
    ...
    public function submit(){
        ...
        // 流程检验
        $pro = D('KkAppflowtable')->havePro('ClientStatementApply_fmh','');
        ...
        if(!M('kk_clientstatement_fmh')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $result = M('kk_clientstatement_fmh')->add($add);
        if(!$result) return array('code' => 404,'msg' => '提交失败，请重新尝试！');
        $copyto_id = trim($copyto_id,',');
        if (!empty($copyto_id)) {
            // 发送抄送消息
            D('KkAppcopyto')->copyTo($copyto_id,'ClientStatementApply_fmh', $result);
        }
        $wf = A('WorkFlow');
        $salesid = session('kk_id');
        $res = $wf->setWorkFlowSV('ClientStatementApply_fmh', $result, $salesid, 'kk');
