...
class KkkpsqApplyLogic extends Model {
    ...
    public function recordContent($id)
    {
        ...
        $result['mydata'] = $res;
        $result['imgsrc'] = '';
        $userInfo = M('kk_boss')->where(array('name'=>$res['kp_jbr']))->select();
    ...
    public function getCustomerList(){
        $data = I('math');
        $res = D('Guest')->getGuest('kk',$data,'kpsq_Apply');
        return $res;
    }
    ...
    {
        $arr = array();
        $arr['kp_bzfs'] = I('kp_bzfs');
        $arr['kp_pz'] = I('kp_pz');
        $arr['clientid'] = I('clientid');
        $arr['client'] = I('client');
        $arr['sday'] = I('sday');
        $arr['eday'] = I('eday');
        $arr['hash'] = hash('md5', $arr[kp_bzfs].$arr['kp_pz'].'fa39@q2@E3rdwaijs.*&&23da(&@#dwa');

        $qpData = $this->curl_request('http://www.fjyuanxin.com/sngl/include/loadKPStatus.php', $arr);
    ...
    public function saveFormData()
    {
        ...
        //sqbh 生成
        $sql = sprintf("select sqbh from  kk_xskp  where year(kp_da)='%s' group by sqbh DESC", $toyear);
        $data = $this->query($sql);
        $sqbh="SQBH".date("Ymd",time()).str_pad((count($data) + 1), 3, '0', STR_PAD_LEFT);
        //pid 生成
        $sql = sprintf("select * from kk_xskp where jl_date >='%s' group by pid DESC", $today);
        $data = $this->query($sql);
        ...
        if(!M('kk_xskp')->autoCheckToken($_POST)) return array('code' => 404,'msg' => '网络延迟，请勿点击提交按钮！');
        $data = M('kk_xskp');
        ...
        if ($result) {
            ...
            if (!empty($copyto_id)) {
                D('kkAppcopyto')->copyTo($copyto_id,'kpsq_Apply', $pid);
            }

            // 签收通知
            $wf = A('WorkFlow');
            $salesid = session('kk_id');
            $wf->setWorkFlowSV('kpsq_Apply', $pid, $salesid, 'kk');
        }else{
    ...
    public function getCustomerInfo()
    {
        ...
        $resArr = array();
        $sql = sprintf("SELECT ht_khmc,ht_dj,ht_yf,ht_bzfs,ht_cate,ht_stat,ht_wlfs FROM kk_ht AS a,kk_guest2 AS b WHERE a.ht_khmc=b.id AND (b.id='%s' or b.reid='%s') AND
 ht_stat='2' AND ht_enday>='%s' AND ht_stday<='%s' GROUP BY ht_wlfs,ht_cate,ht_bzfs", $id, $id, $bigen, $end);
        ...
        $invoiceData = array();
        $sql =sprintf("select ang from kk_dtg as b,kk_guest2 as c where  c.id=b.gid  and ang<>'' and  c.g_name = '%s' GROUP BY gid,ang ORDER BY gid", $userName);
        $data = $this->query($sql);
        if (!empty($data)) {
            $invoiceData = array_merge ($invoiceData, $data);
        }

        $sql = sprintf("SELECT invoice_company as ang FROM kk_invoice_company WHERE delivery_unit = '%s'", $userName);
        $data = $this->query($sql);
        if (!empty($data)) {
            $invoiceData = array_merge ($invoiceData, $data);
        }
    ...
    {
        $map = array('pid' => $id);
        return M('kk_xskp')->where($map)->setField('kp_stat',0);
    }
